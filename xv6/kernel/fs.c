...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  if(ip->type == T_SMALLFILE)
  {
      ...
      //cprintf("memmove dst: %d\n", *((char*)(memmove(dst,(((char*)(&ip->addrs))+off), n))));
  }
else
  {
...
// Write data to inode. n = size of data to write. off = offset to write to. Stored in the file.
int
...
//        cprintf("temp : %d ; ip->addrs+temp : %d\n", temp,*(((char*)(&ip->addrs))+temp));
//      }
      off = off+n;
  }
  else{
      if(off + n > MAXFILE*BSIZE)
        n = MAXFILE*BSIZE - off;
      // This section does the data blocks write
